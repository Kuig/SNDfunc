...
def brightness(x, Fs = 1, envType = 'rms', envParam = 600):
    '''Calculates Equivalent Brightness Frequency (CoBE algo. Basically a real-time instantaneous frequency estimator)'''
    
    '''Presti, G., and D. A. Mauro. "Continuous brightness estimation (CoBE): Implementation and its possible applications." Proceedings of the 10th International Symposium on Computer Music Multidisciplinary Research (CMMR), Marseille, France. Laboratoire de Mecanique et d’Acoustique, 2013.'''
    
    '''Presti, G., D. A. Mauro, and G. Haus. "TRAP: TRAnsient Presence detection exploiting continuous brightness estimation (CoBE)." Proceedings of the 12th International Conference on Sound and Music Computing (SMC-15), Maynooth, Ireland. Music Technology Research Group, Department of Computer Science, Maynooth University, 2015.'''
...
def mfcc(stft, F, n = -1, melbw = 100, upsample = None):
    ...
    mpsd = amp2db(mpsd,-96)
    mfc = sp.fft.dct(mpsd, axis=0, norm='ortho')
    return mfc[0:n,:]
...
##    mpsd = sp.fft.idct(np.vstack((amp2db(sigPow),mfc)), axis=0, norm='ortho')
    mpsd = sp.fft.idct(mfc, axis=0, norm='ortho')
    mpsd = db2amp(mpsd)
    n = mpsd.shape[0]
